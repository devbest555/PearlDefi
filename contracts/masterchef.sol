...
contract PearlToken is BEP20 {
    ...
    IUniswapV2Router02 public pearlSwapRouter;
    // The trading pair
    address public pearlSwapPair;
    // In swap and liquify
    ...
    event pearlSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "PEARL::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the PEARL Token contract.
     */
    constructor() public BEP20("Pearl Token", "PEARL") {
    ...
    /// @dev overrides transfer function to meet tokenomics of Pearl
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(pearlSwapRouter) != address(0)
            && pearlSwapPair != address(0)
            && sender != pearlSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "PEARL::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "PEARL::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the PearlSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = pearlSwapRouter.WETH();

        _approve(address(this), address(pearlSwapRouter), tokenAmount);

        // make the swap
        pearlSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        pearlSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from PearlSwapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "PEARL::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "PEARL::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "PEARL::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updatePearlSwapRouter(address _router) public onlyOperator {
        pearlSwapRouter = IUniswapV2Router02(_router);
        pearlSwapPair = IUniswapV2Factory(pearlSwapRouter.factory()).getPair(address(this), pearlSwapRouter.WETH());
        require(pearlSwapPair != address(0), "PEARL::updatePearlSwapRouter: Invalid pair address.");
        emit pearlSwapRouterUpdated(msg.sender, address(pearlSwapRouter), pearlSwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "PEARL::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "PEARL::delegateBySig: signature expired");
...
// MasterChef, Neptune, is the master of Pearl. He can make Pearl and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract if PEARL is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The maximum supply for PEARL will be 12,000
    uint256 public constant maxSupply = 12000 * 10 ** 18;
    // PEARL tokens created per block.
    uint256 public pearlPerBlock = 0.01 * 10 ** 18;
    // Bonus multiplier for early PEARL makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeAddress;
    // Total PEARL in contract from deposits and rewards
    uint256 private pearlDeposit = 0;
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (pearl.totalSupply() >= maxSupply) {
            return 0;
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 pearlReward = multiplier.mul(pearlPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        if (pearl.totalSupply().add(pearlReward) > maxSupply) {
            pearlReward = maxSupply.sub(pearl.totalSupply());
        }
		uint256 pearlToDev = pearlReward.div(50);
        pearlReward = pearlReward.sub(pearlToDev);
        pearl.mint(devAddress, pearlToDev); // 2% dev fee.
        pearl.mint(address(this), pearlReward);
        pearlDeposit = pearlDeposit.add(pearlReward); // Add pearl minted
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
            ...
            if (address(pool.lpToken) == address(pearl)) {
                uint256 transferTax = _amount.mul(pearl.transferTaxRate()).div(10000);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(pending > 0) {
            safePearlTransfer(msg.sender, pending);
            pearlDeposit = pearlDeposit.sub(pending); // Subtract pearl reward
        }
        if(_amount > 0) {
    ...
    function safePearlTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > pearlBal) {
            transferSuccess = pearl.transfer(_to, pearlBal);
        } else {
            transferSuccess = pearl.transfer(_to, _amount);
        }
    require(transferSuccess, "safePearlTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _pearlPerBlock) public onlyOwner {
        massUpdatePools();
        require(_pearlPerBlock<=10000000000000000000, "You cannot make PEARL Per Block more than 10 PEARL"); // This prevents abuse
        pearlPerBlock = _pearlPerBlock;
